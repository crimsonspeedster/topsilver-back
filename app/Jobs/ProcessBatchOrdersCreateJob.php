...
class ProcessBatchOrdersCreateJob implements ShouldQueue
{
    ...
    {
        $lock = Cache::lock('orders-create-batch-import', 600);
    ...
    private function processChunk(array $orders): array
    {
        ...
        foreach ($orders as $index => $orderData) {
            $publicToken = $orderData['public_token'] ?? null;
            ...
            if (Order::where('public_token', $publicToken)->exists()) {
                $failed++;

                $this->logError(
                    index: $index,
                    code: IntegrationErrorCode::InvalidValue->value,
                    message: 'Order with this public token already exists',
                    externalId: $publicToken,
                );
            ...
            $payment_method = PaymentMethod::where('type', $payment_type)->first();
            $shipping_method = ShippingMethod::where('type', $shipping_type)->first();
            ...
            $shipping_data['shipping_method_type'] = $shipping_method->type->value;
                    ...
                    if (!empty($orderData['items'])) {
                        foreach ($orderData['items'] as $item) {
                            ...
                            if (
                                ...
                                $entity->type === ProductTypes::VARIABLE &&
                                !$variant
                            ) {
                                throw new Exception(
